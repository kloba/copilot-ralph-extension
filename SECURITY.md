...
This is a small, single-maintainer project with a low release cadence. Only the **latest tagged release** receives security updates. If a fix is applied, it ships in the next patch release on top of `main`; older tags are not back-patched.
...
- Every runtime module shipped under `packages/tui/src/` and `packages/tui/bin/`.
- Anything baked into prompt templates that could be exploited by a malicious external input (e.g. a poisoned issue body fed into `--grow-project`).

Out-of-scope:

- Vulnerabilities in the Copilot CLI itself, Node.js, `gh`, or the Ink/React/Yoga/Commander dependency tree. Please report those to the respective projects.
