...
- Every runtime module shipped under `extension/` (currently `extension.mjs`, `handler.mjs`, `events-emit.mjs`; the set is pinned to `install.sh`'s `FILES` array by a drift-guard test, so any future addition lands here automatically).
- `install.sh`
