...
### Features
- New `--fleet` driver mode and bare `autopilot fleet` subcommand for "find one work item, ship it, repeat" autopilot loops. Each iter is atomic (one whole work item end-to-end: orient → implement → commit → push → end) — the agent picks a work item from the three-tier backlog (red CI / stale open PR / open issue), ships it, and emits `COMPLETE`. When all three tiers are empty the agent emits `ABORT_NO_BACKLOG` and the loop terminates cleanly. Unlike `--self-improve` there is NO ideation backstop — fleet drains, it doesn't grow. Bare `autopilot fleet` defaults to `--reset-on=workitem` against the Copilot CLI backend with the existing `--allow-all-tools` yolo wiring (the fleet prompt is validated against the Copilot CLI for v1; backend-agnostic by construction so claude can be enabled later without prompt changes). Stage list `[ORIENT, IMPLEMENT, COMMIT, PUSH, END]` is pinned to the runner's per-iter worktree teardown contract — emitting `[STAGE: END]` at the tail of a successful iter is mandatory.
